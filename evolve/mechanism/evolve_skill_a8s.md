...
3. Respect the KNOWN DEAD-ENDS listed in Phi — do not re-propose a closed direction.

## Heuristic families (self-contained)

Living catalog of strategy families the inner agent can draw from. Meta updates this
section in-place; do not rely on any external family file.

### Active families (try these)
- `pair-baiting` **(DE-ANCHOR priority — try first)**:
  open or pile with a rank you hold in duplicate so the opponent must break pairs or accept
  tempo loss. Scan `RANK_MASK[r]` for ranks with `popcount(L.hand & RANK_MASK[r]) >= 2`;
  on open, attack the *lowest* legal card of such a rank before `pick_cheapest`. In pile phase,
  prefer a throw-in whose rank still has surplus in hand before H3's default. Bump
...
- `card-counting` **(DE-ANCHOR priority — try second)**: memoryless rank exhaustion from
  ...
  visible. Do not use `MemoryFeatures` — B2 passes `memory=nullptr`.
- `trump-economy` / `endgame-tempo`: already embodied in incumbent H1–H5
  (`candidate_0000`). Do not re-tune — see dead-ends below.

### Confirmed dead-ends (do NOT re-propose)
- **H2/H4/H5 scalar gate tweaks** (trump-economy + endgame-tempo): 20 plateau rounds;
  parents through `candidate_0040` produced stepping stones only. Changing
  `deck_count` / `opponent_hand_count` thresholds on existing heuristics is saturated —
  add H6 from a different family instead.
- **H3 pile-rank-matching without pair surplus**: candidates `0025`–`0030` failed keep;
  table-rank throw-in without duplicate-rank pressure is not a new family.
- **Pile-branch re-parent churn without H6** (rounds 37–40): `0037`–`0040` scored
  0.70939 from pile/scalar parents. Switching parent without adding `pick_pair_surplus` + H6
  is not a family change.

## Turn gate (Phi plateau_rounds>=20 — check before you edit)
...
## DE-ANCHOR playbook (when Phi shows `plateau_rounds>=3` or DE-ANCHOR)
...
through `candidate_0040` — see **Heuristic families** section above). Your next edit **must**
add behavioral logic from a *different* family — not another `deck_count` /
`opponent_hand_count` threshold change.
...
except the bounded Phi the harness gives you (best curve, recent outcomes, plateau length,
