...
scripts/
  ...
  diagnose_b4_features.py  summarize a --mode features TSV by macro outcome bucket
...
analysis.ipynb    charts from results.tsv (progress.png, occam.png, score_alignment.png)
                  + optional B4 feature diagnostics (paired with diagnose_b4_features.py)
...
`match --challenger X --opponent Y`, `ablate` (B3 vs B2), `selfplay --strategy X`, and
`features` (B2-vs-opponent per-game diagnostic TSV). Sizes: `--eval quick` (100k seeds) or
`--eval full` (5M seeds); override with `--seeds N`. Progress prints every `--batch` seeds
(default 5000; `--batch 0` to silence). Other flags: `--seed-base`, `--ci normal|bootstrap`.
...
:: Diagnostic feature TSV (one row per individual game)
durak\build\simulate.exe --mode features --challenger B2 --opponent B4 --seeds 100000 --batch 10000 --features-out durak\triage_parts\b4_features_100k.tsv

:: Summarize that feature TSV by macro outcome bucket (win / split / loss)
python scripts\diagnose_b4_features.py durak\triage_parts\b4_features_100k.tsv
```
...
Behavior-neutral simulator instrumentation, such as `--mode features`, is a diagnostic exception for
meta analysis only. Do not treat it as an experiment surface or change evaluation behavior without
explicit authorization in `program.md`.
