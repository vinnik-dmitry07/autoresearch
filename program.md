...
- The human has now authorized **only** the per-deal diagnostic harness. Keep the memory-using B2
  contract change out of scope. The next work is feature analysis, not heuristic grinding.
...
- **jun25 GPT-5.5 Extra High continuation:** Added the notebook model divider and tested `gpt55 b1`, a defense hard-target tie-break that keeps cheapest-beater primary but prefers covering the harder attack card when multiple covers are available. Quick screen: B4 0.69017 / search 0.82543 / lower_ci 0.68833, only +0.00114 B4 / +0.00079 search vs QA. Per gates, discard as noise; strategy restored to `QA`.
- **jun26 diagnostic harness:** Added opt-in simulator feature emission (`--mode features`) plus optional engine tracing. Normal `ladder`/`match`/`ablate` evaluation remains unchanged; feature mode writes one TSV row per individual game and duplicates the paired `pair_score`/`pair_bucket` onto both rows. The smoke run (`--seeds 1000 --batch 200`) reproduced the known residual shape at small N: B2 vs B4 point_rate 0.68650, win 0.5250, loss 0.1060, split 0.3690. Columns are role-normalized (`chal_*`, `opp_*`) and cover initial trump/hand shape, battle count, forced vs voluntary takes, high-trump conservation-take proxies, card flow, pile/table size, draws, and final state.
- **jun26 phase diagnostic pass:** Generated `durak/triage_parts/b4_features_100k.tsv` (`--seeds 100000 --batch 10000`) and added `scripts/diagnose_b4_features.py` plus an optional notebook section. Aggregate check: B2 vs B4 point_rate 0.69017, win 0.5341, loss 0.1070, split 0.3589, matching quick noise around QA. Phase split localizes B4 residual wins/splits to **deck-empty conversion**, not initial deal or early conservation-take failure:
  - Initial trump shape barely moves: `chal_initial_trumps==0` is win 0.1748 / split 0.1927 / loss 0.1850; `chal_initial_high_trumps==0` is win 0.2698 / split 0.2922 / loss 0.2850.
  - Early forced takes are weak: `chal_forced_takes_deck_ge5>=1` is win 0.0659 / split 0.0875 / loss 0.0786.
  - Endgame forced takes dominate: `chal_forced_takes_deck_0>=1` is win 0.0078 / split 0.3142 / loss 0.4868; mean `chal_forced_takes_deck_0` is win 0.0083 / split 0.6139 / loss 0.9441.
  - B4 converts by **not taking** and then throwing trumps at deck empty: `opp_cards_taken==0` is win 0.1710 / split 0.3963 / loss 0.6281; `opp_trump_attack_cards_deck_0>=1` is win 0.0759 / split 0.3369 / loss 0.5598.
  - B2 wins when it gets the deck-empty trump-dump lane instead: `chal_trump_attack_cards_deck_0>=1` is win 0.7874 / split 0.6185 / loss 0.4869; mean `chal_trump_attack_cards_deck_0` drops from 1.4270 in wins to 0.5656 in losses.
  Interpretation: the residual is mostly **initiative/control at deck empty**. B4's memory advantage manifests as reaching the no-deck phase without being forced to take, then converting with trump throw-ins while B2 is forced to take. Do not spend more effort on initial-hand/trump-starvation hypotheses.
- **jun26 first deck-empty forced-take pass:** Extended feature mode with first-event fields for each player's first forced take at `deck==0`: battle, table shape, attack ranks/trumps, defender/attacker hand and trump counts, immediate legal throw-ins, and whether the opponent later makes a deck-empty trump attack. Re-generated the same 100k TSV; aggregate score unchanged (point_rate 0.69017), confirming trace-only behavior. The residual is now sharper:
  - `chal_first_deck0_forced_take` is win 0.0078 / split 0.3142 / loss 0.4868. This single event covers nearly half of B4 pair wins and almost never appears in B2 pair wins.
  - Most of these events are **small-table forced takes**: `chal_first_deck0_forced_take_table<=3` is win 0.0049 / split 0.2865 / loss 0.4337.
  - It is usually **B2 out of trumps at the event**, not initial trump starvation: `chal_first_deck0_forced_take_defender_trumps==0` is win 0.0078 / split 0.2467 / loss 0.3967; conditional on the event, defender trumps average only 0.257 in losses.
  - B4 usually does **not** have immediate non-trump throw-ins at that first event: `chal_first_deck0_forced_take_no_nt_throwins` is win 0.0077 / split 0.3126 / loss 0.4835. Immediate trump throw-in availability is rare (loss 0.0327), so B4's conversion usually happens after the forced take, not as same-battle pile-on.
  - Conditional event means in B4 pair wins: table cards 1.746, attack cards 1.381, trump attacks 0.566, B2 defender hand 1.803, B2 defender trumps 0.257, B4 attacker hand 1.470, B4 attacker trumps 1.164, opponent-later-trump-attack 0.509.
  Interpretation: the residual B4 lane is **one or two card deck-empty attacks that force B2 to take while B2 has no trump cover left; B4 then often converts later with a trump attack**. This is an endgame tempo/last-trump control issue, not an early high-trump conservation threshold issue.
- **jun26 prior-attack / pair-level pass:** Extended the first-event trace with B2's **last deck-empty attack before its first deck-empty forced take**: chosen card rank/trumpness, legal trump/non-trump attack options, and B4's trump holdings at that attack. Re-generated `b4_features_100k.tsv`; aggregate score unchanged. This pass narrows the mechanism:
  - The "B2 had a trump attack but chose non-trump" hypothesis is too small to explain the residual: `chal_prior_attack_chose_nontrump_with_trump_option` is win 0.0055 / split 0.0563 / loss 0.0917.
  - Conditional on the first forced-take event in B4 pair wins, B2's prior deck-empty attack had only 0.495 legal trump options on average; B2 chose a trump only 0.165 of the time, while B4 defender still held 2.431 trumps and 2.220 high trumps on average.
  - The stronger signal is **B4 retaining trump mass through B2's prior deck-empty attack**: `chal_prior_attack_defender_kept_high_trump` is win 0.0073 / split 0.2900 / loss 0.4517.
  - Pair-level counts: B2 pair wins usually have no B2 first forced-take event (52,573/53,407 seeds); splits usually have it in one seat (22,319/35,894 seeds); B4 pair wins often have it in one or both seats (5,806 one-seat, 2,305 two-seat, 2,588 no-event out of 10,699 seeds).
  Interpretation: B2 is usually not simply missing an available trump dump. The residual is more structural: B4 reaches deck-empty defense with multiple/high trumps preserved, B2 cannot force those trumps out with local rank-dump play, and later B2 is forced to take when out of trump cover.
- **jun26 prior-cover response pass:** Added rank/suit and cover-response fields for B2's prior deck-empty attack before its first deck-empty forced take: chosen suit, whether B4 covered that exact slot, cover rank, cover trumpness, same-rank cover, and B4 trump/high-trump counts after cover. Re-generated the 100k TSV; aggregate score stayed `point_rate=0.69017`, confirming trace-only behavior. This pass refines the mechanism:
  - Conditional on the forced-take event in B4 pair wins, B2's prior attack was almost always covered (`0.9409`), usually by a **trump** (`0.7457`), not by same-rank non-trump cover (`0.1746`).
  - B4 still retained trump depth after that cover: pre-cover defender trumps/high-trumps were `2.431/2.220`, and post-cover remained `1.601/1.508`. So the lane is not "B4 simply keeps trumps untouched"; B4 often spends one trump and still has enough high trump mass to force B2 later.
  - Rank view: in B4 pair wins with the event, B2's prior attack rank averaged `3.504` while B4's cover rank averaged `4.505`; the distribution is broad low/mid attacks being covered by higher ranks/trumps, not a single rank bug.
  - No-event B4 pair wins remain a smaller residual slice (`2,588` seeds: `829` pair_score 0.00, `1,759` pair_score 0.25). Without the challenger first deck-empty forced-take sentinel, B2 still takes more cards overall (`chal_cards_taken=5.645` vs `opp_cards_taken=1.665`) and ends with `chal_final_hand_count=1.765`; this looks like a terminal draw/loss boundary rather than the main forced-take lane.
  Interpretation: the main residual is **B4 trump depth after spending trump cover**. Any future candidate rule must identify a local way for B2 to extract multiple B4 trumps or avoid entering the final exchange with B4 still holding 1-2 high trumps; the covered prior attack itself is not enough.
- **jun26 terminal/final-exchange pass:** Added final hand-shape fields (final trump/high-trump/non-trump counts and rank bounds) plus post-prior-cover exchange counters: B2 attacks after B4's cover, B2 legal trump attack options after that cover, and B4 trump/high-trump attacks before B2's first deck-empty forced take. Re-generated the 100k TSV; aggregate score again stayed `point_rate=0.69017`.
  - In B4 pair wins conditional on B2's first deck-empty forced-take event, the prior cover is essentially the last B2 attack: `chal_first_deck0_forced_take_prior_attack_self_attack_cards_after_cover=0`, `self_legal_trump_options_after_cover=0`, and `self_trump_attacks_after_cover=0`.
  - B4's post-cover reserve directly converts: conditional event means in B4 pair wins are `opp_attack_cards_after_cover=1.238`, `opp_trump_attacks_after_cover=0.545`, and `opp_high_trump_attacks_after_cover=0.517`. The threshold `chal_prior_cover_then_opp_high_trump_attack` is win 0.0000 / split 0.1690 / loss 0.2516.
  - The time gap is immediate: `battles_until_forced_take` is `0.882` conditional in B4 pair wins, so B2 usually gets no intervening attack turn after B4 spends the prior cover trump.
  - No-event B4 pair wins are **not** lone-non-trump failures: `chal_lost_with_lone_nontrump=0.0000`. They are often losses/draw-boundaries with B2 still holding trump mass: no-event B4-win means `chal_final_hand_count=1.765`, `chal_final_trumps=0.657`, `chal_final_high_trumps=0.635`; `chal_lost_with_trump_left=0.4569`.
  Interpretation: the main forced-take lane is now localized to a one-exchange lockout: B4 covers B2's last deck-empty attack, then attacks back before B2 has another legal trump-attack opportunity. The no-event lane is a separate terminal-card-order/role-boundary problem where B2 often loses with trumps still stranded in hand.
- **jun26 forced-take snapshot pass:** Added exact bitmask snapshots for the first deck-empty forced-take state: attacker table cards, defender table cards, B2 defender hand, legal defense cards, plus legal trump/non-trump cover counts, coverable/uncoverable uncovered counts, and cheapest-cover metadata. Re-generated the 100k TSV; aggregate score stayed `point_rate=0.69017`.
  - The decisive forced-take is almost never an optional defense/take mistake. Conditional on B2's first deck-empty forced take in B4 pair wins, `legal_defense_cards=0.0028`, `legal_trump_defenses=0.0000`, `coverable_uncovered=0.0023`, and `uncoverable_uncovered=1.0126`.
  - Threshold rates confirm the same: `chal_first_deck0_forced_take_no_legal_defense` is win 0.0078 / split 0.3133 / loss 0.4857, while `any_legal_defense` is only win 0.0000 / split 0.0009 / loss 0.0011 and `legal_trump_defense` is 0.0000 in every bucket.
  - No-event B4 pair wins split into individual-game losses and draws: in the no-event B4-win subset, B2 losing rows (`challenger_points=0.0`) average `final_hand=2.673`, `final_trumps=0.995`, `final_high_trumps=0.962`, and `cards_taken=6.313`; B2 draw rows (`0.5`) have empty final hands by definition and lower `cards_taken=4.347`.
  Interpretation: the forced-take lane is now fully localized to **prior card-order/initiative**, not a missed cover at the forced-take state. B2 is hit with an uncoverable B4 attack after losing the prior exchange. The remaining actionable diagnostic surface is why B2's prior deck-empty attack failed to extract enough B4 trump mass before that attack, and why no-event losses strand roughly one high trump in B2's hand.
- **jun26 decoded snapshot / no-event strata pass:** Extended `scripts/diagnose_b4_features.py` to decode card bitmasks into rank/suit names, print representative B4-win forced-take rows, group the B4 reply by high-trump/low-trump/non-trump, and stratify no-event individual B4 wins by final B2 hand/trump count. This pass used the existing 100k TSV; no simulator rerun was needed.
  - Among B4-win individual losses with the first deck-empty forced-take event and prior attack seen, B4's reply is usually a **high trump**: `reply_high_trump=5,383` rows vs `reply_low_trump=297` and `reply_nontrump=4,058`.
  - Even in the high-trump reply group, B2 often has resources in a broad sense but not a legal cover: `b2_has_reply_rank=0.3468`, `b2_has_trump=0.3270`, prior B2 legal trump options average `0.4609`, and prior B2 trump count averages `0.6779`. Representative rows show these are mostly wrong-suit/small-trump strandings, not a legal-defense miss at the final table.
  - In non-trump reply rows, B2 has no trump at the forced-take state (`b2_has_trump=0.0000`) despite prior B2 trump count averaging `0.7893`, again pointing to the prior exchange consuming/stranding the usable trump route before B4 empties.
  - No-event B4 individual losses stratify as final B2 hand/trumps: `2/0=931`, `2/1=1,237`, `2/2+=470`, `3+/0=121`, `3+/1=270`, `3+/2+=388`. The biggest stratum is **two cards with one high trump plus one non-trump** (`final_high_trumps=0.9814`, `final_nontrumps=1.0000`), not all-junk hands.
  Interpretation: both residual lanes now point at terminal initiative/rank-order rather than defense choice. In the forced-take lane, B4's high trump attack lands after B2's last attack and B2 cannot legally answer. In the no-event lane, B2 commonly still owns a high trump but cannot deploy it before B4 empties.
- **jun26 exact prior-option / final-initiative pass:** Extended trace with exact masks for B2's prior deck-empty legal attack alternatives: all legal attacks, trump/non-trump masks, alternatives that would force B4 to take, alternatives whose cheapest cover is a high trump, and whether the chosen prior attack had those properties. Added deck-empty attack-decision counters for passing or playing non-trump while holding trump, aggregate legal high-trump options, and last-two attack decisions. Re-generated the 100k TSV; aggregate score stayed `point_rate=0.69017`.
  - The main forced-take lane is **not** mostly a missed force-take attack. `chal_prior_attack_had_force_take_option` is win 0.0023 / split 0.0192 / loss 0.0363, and `chal_prior_attack_chosen_forces_take` is 0.0000 in all buckets.
  - B2 usually does make B4 spend a high trump at the prior attack in the residual lane: `chal_prior_attack_had_force_high_trump_cover_option` is win 0.0055 / split 0.2303 / loss 0.3520, and `chal_prior_attack_chosen_forces_high_trump_cover` is win 0.0015 / split 0.1952 / loss 0.2961. Conditional on the forced-take event in B4 pair wins, chosen-forces-high-trump-cover is `0.6083`.
  - Grouped B4-win individual losses with prior attack seen: `reply_high_trump` rows have `force_high_opt=0.8618` and `chosen_force_high=0.7955`; `reply_nontrump` rows have `force_take_opt=0.1858`, `force_high_opt=0.6895`, but `chosen_force_take=0.0000` and `chosen_force_high=0.4904`.
  - Broad deck-empty "pass with trump" / "non-trump with trump" rates are not primary explanations because they are higher in B2 pair wins than B4 pair wins (`chal_deck0_passed_with_trump` win 0.9789 vs loss 0.8062; `chal_deck0_nontrump_with_trump` win 0.9131 vs loss 0.7542). Last-two nontrump/pass-with-trump is only weakly higher in losses (0.4146 vs 0.3868).
  Interpretation: the residual is not a simple missed trump dump or missed take-forcing attack. In the main lane, B2 commonly extracts one B4 high trump, but B4 has enough high-trump depth to answer immediately. The no-event lane and weak last-two signal suggest any candidate rule must be about terminal initiative/card-order under small hands, not a broad "always use trump" rule.
- **jun26 small-hand / exact final-decision pass:** Added final hand masks plus exact last deck-empty attack-decision fields (decision code, chosen card, hand mask, legal attack/trump/high-trump/non-trump masks). Re-generated `b4_features_100k.tsv` with progress; aggregate score stayed `point_rate=0.69017`, so the trace remains behavior-neutral.
  - Small-hand prior-option rows do **not** yield a clean local attack rule. In B4-win individual losses with B2 first deck-empty forced-take event and prior attack seen, when B2's prior hand was `<=2`, a force-high-trump-cover option existed in `0.7992` of rows and B2 already chose one in `0.7550`; force-take options were only `0.0029`. Even after the cover, B4 retained `1.2659` high trumps on average and replied with a high trump in `0.6402` of rows.
  - With prior hand `<=3`, force-high option/chosen rates are `0.7786/0.7075`, force-take option rate is only `0.0319`, and B4 still retains `1.4691` high trumps after cover. The missed-force-high examples are real but too sparse and B4-depth-heavy to justify a broad "choose any high-cover-forcing alternative" rule.
  - The no-event `final_hand=2, final_trumps=1` stratum is now exact: `1,237` rows; B2 is final attacker only `0.3994`; final high trump is still in B2's last deck-empty decision hand in `0.9167`, but is legally attackable/throwable in `0.0000`. The last decision is almost always a pass-with-trump (`code=1`, `0.9321`) with no legal attack mask, not a missed playable high trump.
  Interpretation: both candidate surfaces from the previous note closed. The main event lane is B4 high-trump depth after B2 already extracts one trump; the no-event lane is pure rank/initiative stranding where B2 has a high trump but no legal rank to throw it. A plausible next diagnostic must inspect the *rank that becomes legal before the final pass*: whether B2's earlier deck-empty non-trump/rank choice could have made its high trump rank live before B4 empties.
- **jun26 terminal-rank activation pass:** Added deck-empty rank-activation trace: per-player attack-play counts, ranks played by self/opponent, and B2's last two actual deck-empty attack plays with chosen rank/suit and legal attack/trump/high-trump/non-trump masks. Re-generated `b4_features_100k.tsv`; aggregate score stayed `point_rate=0.69017`.
  - In the no-event `final_hand=2, final_trumps=1` B4 individual-loss stratum (`1,237` rows), `1,214` rows actually have a high final trump. The final high-trump rank was activated by B2 in only `0.0299`, by B4 in `0.2142`, and by either player in `0.2417`, so most final high trumps are stranded because their rank never appears on table at deck empty.
  - The final high trump was not legal at the terminal pass (`0.0000`), but was legal during B2's previous actual deck-empty attack play in `0.5206` and during B2's last actual deck-empty attack play in `0.4624`. This identifies a real missed timing window: B2 often had a legal high trump before the final pass, chose another card, and then lost the rank/initiative route.
  - This is a smaller residual slice: at most roughly `644` individual-game rows in the 100k paired TSV. Even if fully repaired, the expected B4 point-rate lift is probably in the `+0.001` to `+0.003` range, not a new broad mechanism.
  - `choose_attack` can express the candidate without memory/engine leakage: `LocalFeatures` exposes `deck_count`, `opponent_hand_count`, `hand`, table context via `has_done`, and legal attack cards. The likely implementation surface is the optional throw-in/pile branch, where current logic immediately returns the lowest non-trump if one is legal; that is exactly where a legal high trump can be skipped and later stranded.
  Interpretation: there is finally an evidence-backed local candidate, but it must be narrow. The candidate is not "always play trump at deck empty"; broad trump-first probes were already inert/bad. The candidate shape is: when `deck==0`, hand is tiny (`<=3`), opponent is tiny, and a legal **high trump** is available while the current choice would leave that high trump plus unrelated ranks stranded, spend the high trump now rather than preserving it for a rank that may never become legal.
- **jun26 terminal high-trump throw-in probe:** Implemented the narrow candidate in `choose_attack`: during pile/throw-in only (`has_done`), when `deck==0`, `hand_count<=3`, `opponent_hand_count<=2`, and a legal high-trump attack exists, play the lowest legal high trump before defaulting to lowest non-trump/low-trump. Build/tests passed. Result: 200k B4 match `0.69029` CI `[0.68899, 0.69159]`; 1M B4 confirmation `0.68942` CI `[0.68884, 0.69000]`; 200k ladder B4/B1/B0 `0.69029/0.97548/0.98797`, `search_score=0.82538`. This is flat versus the prior quick ledger (`0.82543`) and overlaps the long-run QA plateau, so the probe was discarded and `strategy_heuristic.cpp` restored.
  Interpretation: the terminal high-trump timing signal is real diagnostically but not directly exploitable by a simple local "spend high trump now" override. It likely converts some no-event losses while giving back comparable value in B2 wins/splits where the high trump remains load-bearing. Do not retest this broad tiny-hand high-trump override without a stricter discriminator.
- **jun26 fire-window discriminator pass:** Added behavior-neutral counters for the exact discarded rule's firing condition: deck-empty pile/throw-in decision (`AttackDone` legal), attacker hand `<=3`, opponent hand `<=2`, and at least one legal high-trump attack. Also traced last opportunity hand/opponent size, chosen card, and legal high-trump/non-trump masks. Re-generated `b4_features_100k.tsv`; aggregate stayed `point_rate=0.69017`.
  - The failed rule's opportunity window is **not B4-loss-specific**. It is more common in B2 pair wins than B4 pair wins: opportunity row rate win `0.0597`, split `0.0371`, loss `0.0364`; mean opportunities per row win `0.0704`, split `0.0417`, loss `0.0378`.
  - Existing B2 already chooses the high trump in most of these windows: conditional chosen-high rates are win `0.8209`, split `0.7959`, loss `0.8403`. The discarded probe only changes the chosen-other tail, and that tail is not concentrated in B4 pair wins.
  - By individual result, opportunities are also more common in B2 wins/draws than B2 losses: row rates points `1.0=0.0543`, `0.5=0.0834`, `0.0=0.0218`. This explains why the direct override gave back value despite matching a real no-event loss pattern.
  Interpretation: the terminal high-trump timing signal is a **correlate of active winning endgames**, not a clean repair target. The no-event stranded-trump losses are downstream of broader initiative/rank availability, and local memoryless attack choice cannot reliably tell good high-trump spends from bad ones using only `deck`, hand size, opponent size, and legal high-trump availability.
- **jun26 chosen-other tail closure:** Used the existing fire-window trace to stratify the exact tail the failed high-trump override would change: rows with at least one tiny high-trump opportunity where B2 chose something else. No simulator rerun was needed.
  - The tail also fails the B4-loss-specific gate: chosen-other row rate is win `0.0070`, split `0.0060`, loss `0.0054`; mean chosen-other opportunities per row are win `0.0126`, split `0.0085`, loss `0.0060`.
  - The tail has different semantics by outcome, not a clean bad-choice signature. In B2 pair wins, chosen-other is often a pass (`0.3119`) or non-trump (`0.2195`) with no final hand left; in B4 pair losses it is almost entirely non-trump (`0.8696`), usually with legal non-trump options present (`legal_nt0_rate=0.1304`, so `0.8696` have at least one legal non-trump). That is exactly the current lowest-non-trump policy doing its normal job, not an isolated miss.
  - The most common B4-loss final strata are tiny (`2 cards / 1 high trump = 51 rows`, `2 cards / 2+ high trumps = 49 rows` in the 100k TSV), far too small and not more prevalent than the analogous safe/winning fire-window rows.
  Interpretation: the last plausible ultra-narrow memoryless gate is closed. The remaining residual requires information the memoryless local policy does not have (rank exhaustion / opponent resource state / future initiative), or it is simply irreducible noise around the current B2-vs-B4 plateau.
- **jun26 closure/doc pass:** No new heuristic probes. Updated README evaluation docs to include the optional `--mode features` diagnostic harness. This marks the memoryless local-search branch as closed unless the user explicitly authorizes a stateful/memory branch or a new feature vocabulary.
...
- **(jun25 b2) B2 now wins the majority vs B4 (win 0.5215, split 0.371, loss 0.108).** The remaining
  37% splits and 11% losses were localized by the jun26 per-deal diagnostic harness: the dominant lane
  is deck-empty initiative/card-order, especially B4 preserving enough high-trump depth to answer after
  B2 extracts one cover trump. No memoryless local repair target survived the follow-up probes.
...
**jun26 closure next step:** Stop memoryless local heuristic search unless the user explicitly wants to move beyond the current contract. The diagnostic harness has mapped the B4 residual to terminal initiative and rank/resource information that the local memoryless policy cannot robustly infer.

1. Recommended next research branch is **not another `strategy_heuristic.cpp` tweak**. Either accept the current B2 plateau, or explicitly authorize a memory/stateful B3/B5 branch that can use rank exhaustion/opponent-resource information to decide terminal high-trump timing.
2. If staying strictly memoryless, only run documentation/cleanup/validation: keep the diagnostic harness as optional, preserve `QA`/current accepted policy, and do not re-probe trump-spend, force-cover, final-pass, or tiny-hand fire-window variants without new evidence.
3. Current evidence closes broad take/pass, initial trump count, early-deck shape, immediate pile-on availability, missed legal defense at forced take, broad missed trump attack, missed post-cover trump chance, missed force-take attack, small-hand high-cover-forcing alternative choice, final-pass missed high-trump throw, pure rank-activation-by-opponent hypotheses, the direct tiny-hand high-trump throw-in override, the broader high-trump fire window, and the chosen-other tail as primary explanations.

Historical closed PIVOT ideas remain below for context. Do not re-probe them.
...
```text
- direction: GPT-5.5 Extra High defense hard-target tie-break
  evidence:
    - gpt55 b1 quick: B4 0.69017 / search 0.82543, only +0.00114 B4 / +0.00079 search vs QA.
    - Below discard threshold; same post-QA noise floor as other defense shape/tie-break probes.
  do not retry unless: Target-ordering is paired with a real lookahead/diagnostic contract change.
