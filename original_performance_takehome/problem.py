...
def _fast_machine_ctor():
    try:
        from vliw_native import FastMachine, cpp_available
        if cpp_available():
            return FastMachine
    except OSError:
        return None
...
_FastMachine = _fast_machine_ctor()
...
def Machine(
    ...
    if value_trace is None:
        value_trace = {}
    if trace or _FastMachine is None:
        return PythonMachine(
            mem_dump, program, debug_info, n_cores, scratch_size, trace, value_trace
        )
    return _FastMachine(
        mem_dump,
        program,
...
def myhash(a: int) -> int:
    """A simple 32-bit hash function. Unrolled to match HASH_STAGES."""
    a &= _MASK32
    x = (a + 0x7ED55D16) & _MASK32
    a = (x + ((a << 12) & _MASK32)) & _MASK32
    x = a ^ 0xC761C23C
    a = (x ^ (a >> 19)) & _MASK32
    x = (a + 0x165667B1) & _MASK32
    a = (x + ((a << 5) & _MASK32)) & _MASK32
    x = (a + 0xD3A2646C) & _MASK32
    a = (x ^ ((a << 9) & _MASK32)) & _MASK32
    x = (a + 0xFD7046C5) & _MASK32
    a = (x + ((a << 3) & _MASK32)) & _MASK32
    x = a ^ 0xB55A4F09
    return (x ^ (a >> 16)) & _MASK32
...
def reference_kernel2(mem: list[int], trace: dict[Any, int] | None = None):
    ...
    Pass a dict as trace to record intermediate values. Omit it for the
    final-memory check — that path skips the per-element dict writes.
    """
...
def reference_final(mem: list[int]) -> list[int]:
    """Run the untraced reference and return the final memory image."""
    rounds = mem[0]
    n_nodes = mem[1]
    batch_size = mem[2]
    forest_p = mem[4]
    ...
    for _ in range(rounds):
        for i in range(batch_size):
            idx = mem[idx_p + i]
            val = mh(mem[val_p + i] ^ mem[forest_p + idx])
            nxt = (idx << 1) + (1 if (val & 1) == 0 else 2)
            mem[val_p + i] = val
            mem[idx_p + i] = 0 if nxt >= n_nodes else nxt
    return mem
