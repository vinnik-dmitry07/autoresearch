@echo off
rem Configure + build the VLIW machine with the Visual Studio toolchain.
rem Usage: build.bat            (configure + build, Release)
rem        build.bat fast       (incremental build of the vliw_machine DLL)
...
if "%MODE%"=="fast" (
    ...
    "%CMAKE%" --build "%SRC%build" --target vliw_machine || exit /b 1
) else (
    "%CMAKE%" -S "%SRC%." -B "%SRC%build" -G Ninja -DCMAKE_MAKE_PROGRAM="%NINJA%" -DCMAKE_BUILD_TYPE=Release || exit /b 1
...
if defined RUN_TEST (
    "%CMAKE%" --build "%SRC%build" --target engine_tests --target engine_integration_tests --target profiler_tests || exit /b 1
    pushd "%SRC%build"
