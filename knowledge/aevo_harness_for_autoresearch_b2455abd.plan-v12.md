...
Symbols: `R` = rounds (budget), `K` = inner Pass@K sessions/round, `M` = meta cadence (meta runs every `M` rounds; default 1), `c_quick/c_dual/c_full` = evaluator CPU per gate, `n_adv` = accepted advances (<= R).

- **LLM sessions/round** = `K` inner + (`1` if `round % M == 0` else `0`) meta. Whole run = `R*K + ceil(R/M)`. `M` is the throttle that cuts meta cost and meta-churn.
- **meta-agent sessions/round** = `0..1` bounded `Agent.prompt` (`disabled_tools`: no eval/git/web). Cost = cached prefix + small `Phi` + short edit.
...
- **Reward saturation** (scalar `search_score` stops moving): after `P` rounds with no holdout advance, the harness switches to **de-anchor mode** — the meta prompt drops stale scalar feedback and steers by non-scalar signals (`b(x)`, localized B4 residual), the AEvo "drop stale feedback + diversify" Pi-step. [P1 prompt switch; P2 novelty selection]
