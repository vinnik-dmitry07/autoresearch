...
overview: "Tier A v5 lean: strong init/isolation/result-hygiene spine; detection = cheap backstop (curated marker hashes + stdout + allowlist file scan). Threat model = honest-but-contaminated agent. Template недоступен как файл — primary defense."
todos:
  - id: allowlist-init
...
Agent may read leaked files if present and reuse ideas in code. We do **not** assume path traversal evasion, MOSS-style fingerprint games, or deliberate stdout suppression.
...
Silent structural reuse without markers remains possible (accepted).
```

Tier B read-sandbox — out of scope.
...
- **No shell interpolation** — path list passed via argv/subprocess list, never `$(cat …)` string concat
...
Stage labels: `post_materialize`, `post_scratch`, `post_commit_init`. Fail on any extra file/dir (allowlist verification, not blacklist purge alone).
...
```text
.git is directory inside wt (not file pointer to main)
...
- `run_dir` NOT under `wt` ([`verify_worktree()`](scripts/ladder_lib.py))
...
**Forbidden:** score then invalidate (contaminates `results.tsv`, `archive.json`, logs).

### S6. Scorer temp side-effect isolation

Scorer/eval subprocess cwd = throwaway dir or wt snapshot copy; writes triage artifacts outside canonical results paths until gate passes. Prevents leak-invalid candidate from leaving eval debris in wt.
...
A1/A2/A4/A5 runs from batch `150224` — **contaminated**:

- Move/rename run dirs → `RUN_ROOT/_quarantine/A1_20260629_150224/` (read-only label in manifest)
...
`conservation_take` / homonym `candidate_0019` — **not** gate markers.

Tests: synthetic fixture strings, not real template body.
...
If scorer or post-processing mutated allowlist file → fail closed, no promote.

Optional: lightweight marker re-check on snapshot only if hash differs (should not happen).
...
| Cut | Reason |
|-----|--------|
| Rolling/windowed fingerprints | scaffold FP → security theater |
