...
todos:
  - id: scaffold
    ...
    status: pending
  - id: protect
    content: "[P1] evolver/protect.py = VersionControl: reset every path outside the manifest allowlist back to base after each agent run; capture candidate as git diff vs base + snapshot; inline `git -c user.email=...` commit; reset in finally (crash/Ctrl-C safe); tag accepted advances evo-N. Mechanically replaces the policy-only locked column (program.md:157-166)."
    status: pending
  - id: evaluate
    ...
    status: pending
  - id: store-select
    content: "[P1] External candidate store evolve/candidates/candidate_NNNN/ (model_patch.diff + snapshot + meta.json{parent_id,scores_ref,valid}) written only by the harness + flat archive + evolver/select.py (random | score_child_prop over VALID parents only). Simple size cap. This is the minimum search structure so the external budget explores instead of re-climbing the stuck optimum that caused PLATEAU."
    status: pending
  - id: observe
  ...
  - id: p2-measure-engine
    content: "[P2] EvolutionEngine.step() plug-in interface + GreedyEngine control to MEASURE whether the flat archive beats greedy. Only proceed to richer QD if the archive demonstrably helps."
    status: pending
  - id: p2-qd
...
- **C5 External candidate store + flat archive + selection** — `evolver/store.py` (or fold into loop), `evolver/select.py`, `evolve/candidates/`.
  - Closes: candidates living in the agent's head; and greedy re-climb of the stuck optimum (the literal cause of PLATEAU). Every valid candidate kept as a branchable stepping stone; parent sampled `random`/`score_child_prop` over **valid** parents only.
  - Test: archive persists across rounds; a non-best parent is sometimes chosen; the reported best advances only under the locked keep rule.
  - Risk: archive growth/dilution -> simple size cap in Phase 1; full Curator is Phase 2.
- **C6 Seed-disjoint holdout gate + keep rule** — reuse `scripts/triage.bat dual`; keep rule in `evolver/evaluate.py`.
  - Closes: keeping noise / a lucky `+eps` (a self-scoring-adjacent failure). Escalate on seed_base=0; advance the reported best only after it also clears the keep rule on the disjoint seed_base=1 holdout + full (`program.md:246-251`); regressions roll back.
  - Test: a candidate that beats seed-0 but not seed-1 is **not** kept; keep requires `search_full >= best+0.005 AND lower_ci >= best_lower_ci`.
...
- **EvolutionEngine plug-in + GreedyEngine control** (`evolver/engine/base.py`, `greedy.py`) — closes "we never proved the archive helps"; lets engines swap without touching the loop. Risk: premature abstraction -> introduced only when the second engine exists.
- **Quality-diversity** (`evolver/descriptors.py`, `population/{map_elites,islands}.py`, richer `select.py` + `select_policy.json`) — MAP-Elites grid, islands+migration, gridless novelty/curiosity/modifiable selection over `b(x)` from `--mode features`. Closes diversity collapse beyond flat. Risk: noise fragments cells -> coarse bins + `lower_ci` admission.
...
- **Phase-1 search = flat archive + `score_child_prop`**, not greedy — greedy is the structure that plateaued (HyperAgents), kept only as a Phase-2 control.
...
Tiny-budget dry run (`max_rounds=2`, `K=2`, quick+dual gate) asserting the core's guarantees:
- the run executes the **full budget** and ends only because the budget is spent (it continues past an inner "we're done");
...
- a candidate that beats the seed-0 search bank but **fails the seed-1 holdout is rolled back**;
- an **inner-session crash becomes an invalid candidate** without killing the loop (reset in `finally`);
- a **non-best parent** is sometimes selected; an accepted advance gets an **`evo-N` tag**.
