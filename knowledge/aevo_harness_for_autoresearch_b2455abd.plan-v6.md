...
todos:
  ...
  - id: agents
    ...
    status: pending
  - id: mechanism-files
    content: "[CORE] Author lean mechanism files (meta evolvable layer): meta_skill.md + evolve_skill.md (contract + AEvo anti-stop + how to read population/history, NOT over-prescriptive), feedback_spec.md, select_policy.json, seed family_map.md/memory.md/next_goal.md. [OPT-IN] a curated skills/ library where the meta agent distills reusable search tactics after notable outcomes (Hermes, description-first). Meta agent may switch engine/container/selector within the manifest allowlist (opt-in layers off by default)."
    status: pending
  - id: loop
