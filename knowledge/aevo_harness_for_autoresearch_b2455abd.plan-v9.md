...
todos:
  ...
  - id: evaluate
    ...
    status: pending
  - id: store-select
    content: "[P1] MANDATORY open-ended branching archive: evolve/candidates/candidate_NNNN/ (model_patch.diff vs base + snapshot + meta.json{parent_id,base_commit,valid,n_children,alpha,scores_ref,status}) written only by the harness; keep ALL valid candidates incl. regressions as stepping stones (never deleted). evolver/select.py default score_child_prop: s=sigmoid(10*(alpha-mean_top3)); h=1/(1+n_children); p∝s*h; sample K with replacement over valid; alpha=HOLDOUT score (paper.txt:208,374-425). random=ablation; greedy=ablation only (w/o-open-ended = little to no progress, paper.txt:199)."
    status: pending
  - id: observe
